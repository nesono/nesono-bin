...
if len(sys.argv) == 1:
    print(
        "usage: %s -f template new.jail.name" % os.path.basename(sys.argv[0])
    )
    sys.exit(1)
...
def get_if_table(jail_interface):
    interfaces = sp.check_output(
        "ifconfig " + jail_interface + " | grep inet", shell=True
    ).splitlines()
...
def assert_jail_with_ip(ip):
    names = sp.check_output(
        "ezjail-admin list | awk '{print $3, $4}'", shell=True
    ).splitlines()[2:]
    jail_table = dict(
        (k.strip(), v.strip()) for k, v in (item.split(" ") for item in names)
    )
    if ip in jail_table:
...
if not if_free:
    print(
        "no interface for jails available, creating a new interface alias for interface %s"
        % jail_if
    )
...
sp.check_output(
    "ezjail-admin create %s %s" % (" ".join(sys.argv[1:]), next_ip), shell=True
)
...
print("starting jail")
sp.check_output(
    "ezjail-admin start %s" % sys.argv[-1].replace(".", "_"), shell=True
)
